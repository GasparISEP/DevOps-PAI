        ...
        fetch(url)
            .then(response => {
                if (response.status === 404) {
                    throw new Error("No average grade available for the selected Course Edition.");
                }
                if (!response.ok) {
                    throw new Error("Unexpected error while fetching average grade.");
                }
                return response.json();
            })
            .then(data => {
                setAverageGrade(data.averageGrade); // may be null
